...
public class UserDB extends ManagerImplDB implements Dao<User> {
	 ...
	 */
	@Override
	public boolean create(final User t) {
		try {
			open();
	 ...
	 */
	@Override
	public boolean update(final User t) {
		try {
			open();
			prepared = super.getConn().prepareStatement(
					"update UTENTE set nome= ?, password= ?, email= ?,isModeratore= ? where idUser= ?");
			prepared.setString(1, t.getUsername());
			prepared.setString(2, crp.Crypt(t.getPassword()));
			prepared.setString(3, crp.Crypt(t.getEmail()));
			prepared.setBoolean(4, t.isModerator());
